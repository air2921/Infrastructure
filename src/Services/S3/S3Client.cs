...
public class S3Client(IAmazonS3 s3Client, ILoggerEnhancer<S3Client> logger) : IS3Client
{
    ...
    public ValueTask<S3ObjectUrlDetails> SignUrlAsync(string bucket, string key, DateTime expires)
    {
        ...
        {
            logger.LogError(ex, "An error occurred while attempting to create a signed reference to an object", key, bucket);
    ...
    public async Task<S3ObjectContentDetails> DownloadAsync(string bucket, string key, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "An error occurred while trying to unload an object from storage", key, bucket);
    ...
    public async Task UploadAsync(Stream stream, string bucket, string key, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "An error occurred while trying to load the object into storage", key, bucket);
    ...
    public async Task RemoveAsync(string bucket, string key, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "An error occurred while attempting to delete an object from storage", key, bucket);
